...
public class MessageCompose extends K9Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (K9.getK9ComposerThemeSetting() != K9.Theme.USE_GLOBAL) {
			...
			View v = ((LayoutInflater) wrapper
					.getSystemService(Context.LAYOUT_INFLATER_SERVICE))
					.inflate(R.layout.message_compose, null);
			TypedValue outValue = new TypedValue();
			// background color needs to be forced
			wrapper.getTheme().resolveAttribute(
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		ArrayList<Parcelable> attachments = savedInstanceState
	...
	private TextBody buildText(boolean isDraft,
		 ...
		 * opens a saved draft.
		 */
		boolean includeQuotedText = (mQuotedTextMode
				.equals(QuotedTextMode.SHOW) || isDraft);

		// Reply after quote makes no sense for HEADER style replies
		boolean replyAfterQuote = (mQuoteStyle == QuoteStyle.HEADER) ? false
				: mAccount.isReplyAfterQuote();

		boolean signatureBeforeQuotedText = mAccount
		...
		if (messageFormat == SimpleMessageFormat.HTML) {
			...
			if (includeQuotedText && mQuotedHtmlContent != null) {
				if (K9.DEBUG) {
					Log.d(K9.LOG_TAG,
							"insertable: " + mQuotedHtmlContent.toDebugString());
				}
	...
	private void addAttachmentsToMessage(final MimeMultipart mp)
		...
		for (int i = 0, count = mAttachments.getChildCount(); i < count; i++) {
			...
			MimeBodyPart bp = new MimeBodyPart(
					new LocalStore.LocalAttachmentBody(attachment.uri,
							getApplication()));
			...
			bp.addHeader(MimeHeader.HEADER_CONTENT_TYPE, String.format(
					"%s;\n name=\"%s\"", attachment.contentType, EncoderUtil
		...
		if (identityString.charAt(0) == IDENTITY_VERSION_1.charAt(0)
			...
			if (K9.DEBUG)
				Log.d(K9.LOG_TAG, "Got a saved legacy identity: "
						+ identityString);
			StringTokenizer tokens = new StringTokenizer(identityString, ":",
					false);
			...
			if (tokens.hasMoreTokens()) {
				identity.put(IdentityField.SIGNATURE,
						Utility.base64Decode(tokens.nextToken()));
			}
			if (tokens.hasMoreTokens()) {
	...
	private void addNoteInContentMessage(){
		this.mMessageContentView.append("\n\nEsta e-mail possui mensagens de ‡udio em anexo.");
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.add_attachment:
			onAddAttachment();
			break;
		case R.id.add_record_attachment:
			onAddRecordAttachment();
			break;
	...
	private void processSourceMessageText(Message message, Integer bodyOffset,
		...
		if (textPart != null) {
			...
			if (bodyLength != null && bodyLength + 1 < text.length()) { // + 1
				...
				if (bodyOffset == 0
						&& text.substring(bodyLength, bodyLength + 2).equals(
								"\n\n")) {
	...
	private void populateUIWithQuotedMessage(boolean showQuotedText)
		...
		if (mQuotedTextFormat == SimpleMessageFormat.HTML) {
			...
			mQuotedText.setText(quoteOriginalTextMessage(
					mSourceMessage,
	...
	private String getBodyTextFromMessage(final Message message,
		...
		if (format == SimpleMessageFormat.HTML) {
			...
			if (part != null) {
				...
				return HtmlConverter.textToHtml(
						MimeUtility.getTextFromPart(part), true);
			}
		...
		if (quoteStyle == QuoteStyle.PREFIX) {
			...
			if (originalMessage.getSentDate() != null) {
				quotedText
						...
						.append(" ").append(originalMessage.getSentDate())
						.append("\n");
			}
			if (originalMessage.getRecipients(RecipientType.TO) != null
				...
				quotedText
						...
						.append(Address.toString(originalMessage
								.getRecipients(RecipientType.TO))).append("\n");
			}
			if (originalMessage.getRecipients(RecipientType.CC) != null
				...
				quotedText
						...
						.append(" ")
						.append(Address.toString(originalMessage
								.getRecipients(RecipientType.CC))).append("\n");
			}
	...
	private InsertableHtmlContent quoteOriginalHtmlMessage(
		...
		if (quoteStyle == QuoteStyle.PREFIX) {
			...
			header.append("<hr style='border:none;border-top:solid #E1E1E1 1.0pt'>\n"); // This
																						...
																						// text
																						// conversion.
			if (mSourceMessage.getFrom() != null
					&& Address.toString(mSourceMessage.getFrom()).length() != 0) {
				header.append("<b>")
						.append(getString(R.string.message_compose_quote_header_from))
						.append("</b> ")
						.append(HtmlConverter.textToHtmlFragment(Address
								.toString(mSourceMessage.getFrom())))
						.append("<br>\n");
			}
			if (mSourceMessage.getSentDate() != null) {
				header.append("<b>")
						.append(getString(R.string.message_compose_quote_header_send_date))
						.append("</b> ").append(mSourceMessage.getSentDate())
						.append("<br>\n");
			}
			...
			if (mSourceMessage.getSubject() != null) {
				header.append("<b>")
						.append(getString(R.string.message_compose_quote_header_subject))
						.append("</b> ")
						.append(HtmlConverter.textToHtmlFragment(mSourceMessage
